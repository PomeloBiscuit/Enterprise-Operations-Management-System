...
if (can_view_business_data()) { // 管理員與內部員工權限
    ...
    $L_addCustomer = t('customer.add.title');
    ...
    <div style='background-color: white; padding: 20px; border-radius: 10px; box-shadow: 0 4px 10px rgba(0, 0, 0, 0.1); width: 100%;'> <!-- 顯示顧客列表 -->
        <h3 style='text-align: center; font-family: \"Noto Sans TC\", \"Times New Roman\", serif;'>$L_title</h3><hr> <!-- 顯示標題 -->
        ...
        <form method='post' action='' style='display: flex; justify-content: center; align-items: center; gap: 10px; margin-bottom: 20px;'> <!-- 搜尋表單 -->
            ...
            <select name='searchColumn' class='form-select' style='max-width: 200px;'> <!-- 搜尋條件下拉選單 -->
                ...
                <option value='all' " . ($searchColumn === 'all' ? 'selected' : '') . ">$L_allColumns</option> <!-- 搜尋全部欄位 -->
            ...
            <a href='index.php?Act=320&resultsPerPage=$resultsPerPage' class='btn btn-success'>$L_addCustomer</a> <!-- 新增按鈕 -->
        </form>
            ...
            foreach ($results as $row) { // 逐筆顯示查詢結果
                ...
                <tr> <!-- 顯示顧客資料 -->
                    ...
                    <td style='text-align: center;'> <!-- 操作按鈕 -->
                        <a href='index.php?Act=340&id={$row['CustomerID']}&resultsPerPage=$resultsPerPage' class='btn btn-primary btn-sm'>$L_edit</a> <!-- 編輯按鈕 -->
                    </td> <!-- 結束操作按鈕 -->
                </tr>
                ";
            }
    ...
    echo "<p style='text-align:center; color:red;'>" . t('common.permission_denied') . "</p>"; // 權限不足
}
?>
...
function confirmDelete() { // 確認刪除
    ...
    if (checkboxes.length === 0) { // 未選擇任何顧客
        alert(<?php echo json_encode(t('customer.none_selected')); ?>);
        return false; // 取消提交
    }
    return confirm(<?php echo json_encode(t('customer.confirm_delete')); ?>);
}
