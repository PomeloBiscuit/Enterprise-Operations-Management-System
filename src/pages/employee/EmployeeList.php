...
if (can_view_business_data()) { // 管理員與內部員工才可檢視業務資料
    ...
    $L_addEmployee = t('employee.add.title');
    ...
    <div style='background-color: white; padding: 20px; border-radius: 10px; box-shadow: 0 4px 10px rgba(0, 0, 0, 0.1); width: 100%;'>
        ...
        <form method='post' action='' style='display: flex; justify-content: center; align-items: center; gap: 10px; margin-bottom: 20px;'>
            ...
            <a href='index.php?Act=360&resultsPerPage=$resultsPerPage' class='btn btn-success'>$L_addEmployee</a>
        </form>
            ...
            foreach ($results as $row) { // 逐筆顯示
                ...
                <tr>
                    ...
                    <td style='text-align: center;'>
                        <a href='index.php?Act=380&id={$row['EmployeeID']}&resultsPerPage=$resultsPerPage' class='btn btn-primary btn-sm'>$L_edit</a>
                    </td>
                </tr>
                ";
            }
...
function confirmDelete() {
    ...
    if (checkboxes.length === 0) {
        alert(<?php echo json_encode(t('employee.none_selected')); ?>);
        return false;
    }
    return confirm(<?php echo json_encode(t('employee.confirm_delete')); ?>);
}
</script>
